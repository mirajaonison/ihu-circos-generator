...
if __name__ == '__main__':
    ...
    group_increment = 1
    rows = []
    for _,group in data.groupby("organisms"):
        organism = group['organisms'].values[0]
        subgroups = split_dataframe(group, unit)
        for subgroup in subgroups:
            z = defaultZ if len(subgroup) == unit else math.ceil((len(subgroup)/unit)*defaultZ)
            groupId = "group%i" % group_increment
            row = [groupId, organism, z]
            rows.append(row)
            group_increment+=1
    # Karyotype
    groups['genes'] = groups['genes'].transform(lambda subgroup: unit + math.ceil(subgroup/unit) if math.ceil(subgroup/unit) < unit else math.ceil(subgroup/unit))
    karyotypes = []
    links = []
    ...
    for i,row in enumerate(rows):
        ...
        link = [row[0], 1, 1, row[1], groupstart, groupend, "color=color%s,z=%i" % (row[1].lower(), row[2])]
        links.append(link)
